(function (factory) {
	...
	*/
	...
	const typewritter = document.querySelectorAll("h2[data-title]"); // typewritter.forEach((el) => {
	...
	const navLi = document.querySelectorAll(".Navigation-list.-menu li"); // const sectionTitle = [...document.querySelectorAll(".Section-Content-Title")];
	...
	observerElements.forEach(el => {
	  ...
	  el.observer = new IntersectionObserver(entry => {
	    if (entry[0].intersectionRatio > 0) {
	      gsapWithCSS.ticker.add(el.progressTween);
	    } else {
	      gsapWithCSS.ticker.remove(el.progressTween);
	    }
	  }, observerOptions);

	  el.progressTween = () => {
	    // Get scroll distance to bottom of viewport.
	    const scrollPosition = window.scrollY + window.innerHeight; // Get element's position relative to bottom of viewport.

	    const elPosition = scrollPosition - el.offsetTop; // Set desired duration.

	    const durationDistance = window.innerHeight + el.offsetHeight; // Calculate tween progresss.

	    const currentProgress = elPosition / durationDistance; // Set progress of gsap timeline.
